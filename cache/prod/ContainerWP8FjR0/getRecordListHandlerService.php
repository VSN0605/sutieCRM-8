<?php

namespace ContainerWP8FjR0;

use Symfony\Component\DependencyInjection\Argument\RewindableGenerator;
use Symfony\Component\DependencyInjection\ContainerInterface;
use Symfony\Component\DependencyInjection\Exception\RuntimeException;

/*
 * @internal This class has been auto-generated by the Symfony Dependency Injection Component.
 */
class getRecordListHandlerService extends App_KernelProdContainer
{
    /*
     * Gets the private 'App\Data\LegacyHandler\RecordListHandler' shared autowired service.
     *
     * @return \App\Data\LegacyHandler\RecordListHandler
     */
    public static function do($container, $lazyLoad = true)
    {
        include_once \dirname(__DIR__, 3).''.\DIRECTORY_SEPARATOR.'core'.\DIRECTORY_SEPARATOR.'backend'.\DIRECTORY_SEPARATOR.'Data'.\DIRECTORY_SEPARATOR.'Service'.\DIRECTORY_SEPARATOR.'RecordListProviderInterface.php';
        include_once \dirname(__DIR__, 3).''.\DIRECTORY_SEPARATOR.'core'.\DIRECTORY_SEPARATOR.'backend'.\DIRECTORY_SEPARATOR.'Data'.\DIRECTORY_SEPARATOR.'LegacyHandler'.\DIRECTORY_SEPARATOR.'RecordListHandler.php';
        include_once \dirname(__DIR__, 3).''.\DIRECTORY_SEPARATOR.'core'.\DIRECTORY_SEPARATOR.'backend'.\DIRECTORY_SEPARATOR.'Data'.\DIRECTORY_SEPARATOR.'LegacyHandler'.\DIRECTORY_SEPARATOR.'BaseListDataHandler.php';
        include_once \dirname(__DIR__, 3).''.\DIRECTORY_SEPARATOR.'core'.\DIRECTORY_SEPARATOR.'backend'.\DIRECTORY_SEPARATOR.'Data'.\DIRECTORY_SEPARATOR.'LegacyHandler'.\DIRECTORY_SEPARATOR.'ListDataHandlerInterface.php';
        include_once \dirname(__DIR__, 3).''.\DIRECTORY_SEPARATOR.'core'.\DIRECTORY_SEPARATOR.'backend'.\DIRECTORY_SEPARATOR.'Data'.\DIRECTORY_SEPARATOR.'LegacyHandler'.\DIRECTORY_SEPARATOR.'ListDataHandler.php';
        include_once \dirname(__DIR__, 3).''.\DIRECTORY_SEPARATOR.'core'.\DIRECTORY_SEPARATOR.'backend'.\DIRECTORY_SEPARATOR.'Data'.\DIRECTORY_SEPARATOR.'LegacyHandler'.\DIRECTORY_SEPARATOR.'PresetListDataHandlers.php';

        $a = ($container->privates['App\\Data\\Service\\Record\\EntityRecordMappers\\EntityRecordMapperRunner'] ?? $container->load('getEntityRecordMapperRunnerService'));

        if (isset($container->privates['App\\Data\\LegacyHandler\\RecordListHandler'])) {
            return $container->privates['App\\Data\\LegacyHandler\\RecordListHandler'];
        }

        return $container->privates['App\\Data\\LegacyHandler\\RecordListHandler'] = new \App\Data\LegacyHandler\RecordListHandler(\dirname(__DIR__, 3), (\dirname(__DIR__, 3).'/public/legacy'), 'SCRMSESSID', 'SCRMSESSID', ($container->privates['App\\Engine\\LegacyHandler\\LegacyScopeState'] ??= new \App\Engine\LegacyHandler\LegacyScopeState()), ($container->privates['App\\Module\\LegacyHandler\\ModuleNameMapperHandler'] ?? self::getModuleNameMapperHandlerService($container)), new \App\Data\LegacyHandler\ListDataHandler(($container->privates['App\\Data\\LegacyHandler\\FilterMapper\\LegacyFilterMapper'] ?? $container->load('getLegacyFilterMapperService')), ($container->privates['App\\Data\\LegacyHandler\\RecordMapper'] ?? $container->load('getRecordMapperService'))), new \App\Data\LegacyHandler\PresetListDataHandlers(new RewindableGenerator(function () use ($container) {
            yield 0 => ($container->privates['App\\Data\\LegacyHandler\\PresetDataHandlers\\AlertsDataHandler'] ?? $container->load('getAlertsDataHandlerService'));
            yield 1 => ($container->privates['App\\Data\\LegacyHandler\\PresetDataHandlers\\HistoryTimelineDataHandler'] ?? $container->load('getHistoryTimelineDataHandlerService'));
            yield 2 => ($container->privates['App\\Data\\LegacyHandler\\PresetDataHandlers\\MultiModuleDataHandler'] ?? $container->load('getMultiModuleDataHandlerService'));
            yield 3 => ($container->privates['App\\Data\\LegacyHandler\\PresetDataHandlers\\OutboundEmailDataHandler'] ?? $container->load('getOutboundEmailDataHandlerService'));
            yield 4 => ($container->privates['App\\Data\\LegacyHandler\\PresetDataHandlers\\ProspectListDataHandler'] ?? $container->load('getProspectListDataHandlerService'));
            yield 5 => ($container->privates['App\\Data\\LegacyHandler\\PresetDataHandlers\\SubpanelDataHandler'] ?? $container->load('getSubpanelDataHandlerService'));
        }, 6)), ($container->services['request_stack'] ??= new \Symfony\Component\HttpFoundation\RequestStack()), $a, ($container->privates['App\\Module\\LegacyHandler\\ModuleRegistryHandler'] ?? $container->load('getModuleRegistryHandlerService')));
    }
}
